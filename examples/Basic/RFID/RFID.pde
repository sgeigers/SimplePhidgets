/*
 This example shows simple use of an Phidget RFID board
*/
...
void draw() {
  ...
  if (myRFID.read() == 1) {
    strokeWeight(5);
    // check if the present tag is a specific one. This string can be obtained using the Phidgets Control Panel.
    if (myRFID.getLastTagString() == "ADFKALFK") {
      fill(255,255,0);
    }
    else {
      noFill();

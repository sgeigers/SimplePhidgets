/*
 This example demonstrates the use of text LCD board (the "1203", which is an interfaceKit integrated with a 2X20 characters text LCD)
 NOTICE: After making any changes to the displayed text, you must use the flush() function in order to redraw the LCD with those changes.
*/
...
void setup() {
  size(400, 400);
  myLCD = new Channel(this, "1203");  // opening a channel for the LCD board
  myLCD.setBacklight(0.7);            // turning on the backlight with 70% intensity 
}

void draw() {
  if (mousePressed) {
    myLCD.clear();                              // delete all characters from screen
    myLCD.writeText(0,0,"There are swans in");  // write text to line 0 (top)
    myLCD.writeText(0,1,"the lake!");           // write text to line 1 (bottom)
    myLCD.flush();                              // refresh the display with new text
  }
  else {
    myLCD.clear();                              // delete all characters from screen
    myLCD.writeText(0,0,"Good morning World!"); // write text to line 0 (top)
    myLCD.flush();                              // refresh the display with new text
  }    
}
...
/*
 ...
 setCharacterBitmap(String character, byte[] bitmap) - set a user specified character. see example Advanced_Concepts -> Text_LCD_Advanced
